...
static bs_t* bs_new(uint8* buf, size_t size);
static void bs_free(bs_t* b);
static bs_t* bs_clone( bs_t* dest, const bs_t* src );
static bs_t*  bs_init(bs_t* b, uint8* buf, size_t size);
static uint32 bs_byte_aligned(bs_t* b);
static int bs_eof(bs_t* b);
static int bs_overrun(bs_t* b);
static int bs_pos(bs_t* b);

static uint32 bs_peek_u1(bs_t* b);
static uint32 bs_read_u1(bs_t* b);
static uint32 bs_read_u(bs_t* b, int n);
static uint32 bs_read_f(bs_t* b, int n);
static uint32 bs_read_u8(bs_t* b);
static uint32 bs_read_ue(bs_t* b);
static int32  bs_read_se(bs_t* b);

static void bs_write_u1(bs_t* b, uint32 v);
static void bs_write_u(bs_t* b, int n, uint32 v);
static void bs_write_f(bs_t* b, int n, uint32 v);
static void bs_write_u8(bs_t* b, uint32 v);
static void bs_write_ue(bs_t* b, uint32 v);
static void bs_write_se(bs_t* b, int32 v);

static int bs_read_bytes(bs_t* b, uint8* buf, int len);
static int bs_write_bytes(bs_t* b, uint8* buf, int len);
static int bs_skip_bytes(bs_t* b, int len);
static uint32 bs_next_bits(bs_t* b, int nbits);
// IMPLEMENTATION

static inline bs_t* bs_init(bs_t* b, uint8* buf, size_t size)
{
    b->start = buf;
    b->p = buf;
    b->end = buf + size;
...
static inline bs_t* bs_new(uint8* buf, size_t size)
{
...
static inline uint32 bs_byte_aligned(bs_t* b) 
{ 
...
static inline uint32 bs_read_u1(bs_t* b)
{
    uint32 r = 0;
...
static inline uint32 bs_peek_u1(bs_t* b)
{
    uint32 r = 0;
...
static inline uint32 bs_read_u(bs_t* b, int n)
{
    uint32 r = 0;
    int i;
    for (i = 0; i < n; i++)
...
static inline uint32 bs_read_f(bs_t* b, int n) { return bs_read_u(b, n); }

static inline uint32 bs_read_u8(bs_t* b)
{
#ifdef FAST_U8
...
static inline uint32 bs_read_ue(bs_t* b)
{
    int32 r = 0;
    int i = 0;
...
static inline int32 bs_read_se(bs_t* b) 
{
    int32 r = bs_read_ue(b);
    if (r & 0x01)
    {
        r = (r+1)/2;
    }
...
static inline void bs_write_u1(bs_t* b, uint32 v)
{
    b->bits_left--;
...
static inline void bs_write_u(bs_t* b, int n, uint32 v)
{
    int i;
    for (i = 0; i < n; i++)
    {
...
static inline void bs_write_f(bs_t* b, int n, uint32 v) { bs_write_u(b, n, v); }

static inline void bs_write_u8(bs_t* b, uint32 v)
{
#ifdef FAST_U8
...
static inline void bs_write_ue(bs_t* b, uint32 v)
{
...
static inline void bs_write_se(bs_t* b, int32 v)
{
    if (v <= 0)
    {
        bs_write_ue(b, -v*2);
...
static inline int bs_read_bytes(bs_t* b, uint8* buf, int len)
{
    int actual_len = len;
...
static inline int bs_write_bytes(bs_t* b, uint8* buf, int len)
{
    int actual_len = len;
    if (b->end - b->p < actual_len) { actual_len = (int)(b->end - b->p); }
    if (actual_len < 0) { actual_len = 0; }
...
static inline uint32 bs_next_bits(bs_t* bs, int nbits)
{
   bs_t b;
   bs_clone(&b,bs);
...
static inline uint64 bs_next_bytes(bs_t* bs, int nbytes)
{
   int i = 0;
   uint64 val = 0;
...
inline void bs_trace(const char * fmt, ...)
{
