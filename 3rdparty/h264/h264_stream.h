...
int find_nal_unit(uint8* buf, int size, int* nal_start, int* nal_end);

int rbsp_to_nal(const uint8* rbsp_buf, const int* rbsp_size, uint8* nal_buf, int* nal_size);
int nal_to_rbsp(const uint8* nal_buf, int* nal_size, uint8* rbsp_buf, int* rbsp_size);

int read_nal_unit(h264_stream_t* h, uint8* buf, int size);
int peek_nal_unit(h264_stream_t* h, uint8* buf, int size);
